...
func (t *SimpleChaincode) Init(stub shim.ChaincodeStubInterface) pb.Response  {
	...
	account.Role = args[1]
	account.Balance, err = strconv.Atoi(args[2])
	if err != nil {
...
func (t *SimpleChaincode) B2CPayCoin (stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	ouyeelbytes, err := stub.GetState("ouyeel")
	if err != nil {
		return shim.Error("Failed to get state")
	}
	if ouyeelbytes == nil {
	...
	err = stub.PutState(exchange.Changedate, exchangeBytes)
	if err != nil {
		return shim.Error(err.Error())
	}
	
    return shim.Success([]byte("兑换成功"))
...
func (t *SimpleChaincode) C2CPayCoin (stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	err = stub.PutState(transfer.Amount, transferBytes)
	if err != nil {
		return shim.Error(err.Error())
	}
